...
def getSuggestions(isochrones: list, placetype: str,
    ...
    radius_m = min(common['intersection_radius_km'] * 1000, 50000)  # Convert to meters, capped at 50 km.
    polygons = common['intersection_polygons']

    # 2. Use Places API to search within the enclosing circle of the intersection.
    location = (center_lat, center_lng)
    search_radius = max(int(radius_m), 100)  # Minimum search radius is 100 m.
...
def queryAI(suggestions: list) -> str:
    ...
    for i, poi in enumerate(suggestions, 1):
        ...
        if travel_details:
            ...
            lines.extend(travel_details[:5])  # Show at most 5 people.
